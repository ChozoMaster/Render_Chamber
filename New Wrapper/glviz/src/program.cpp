...
UniformVoxelConfiguration::UniformVoxelConfiguration()
: glUniformBuffer(1 * sizeof(GLfloat))
{
}

void
UniformVoxelConfiguration::set_buffer_data(float const* size)
{
	bind();
//	glBufferData(GL_UNIFORM_BUFFER, 1 * sizeof(GLfloat), size,
//		GL_DYNAMIC_DRAW);
	glBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(GLfloat), size);
	unbind();
}
...
ProgramMesh3::ProgramMesh3()
	: glProgram(), m_wireframe(false), m_smooth(true)
{
	initialize_shader_obj();
	initialize_program_obj();
}
...
ProgramComputeShader::ProgramComputeShader(const unsigned char src[])
: glProgram()
{
	initialize_shader_obj(src);
	initialize_program_obj();
}


void
ProgramComputeShader::initialize_shader_obj(const unsigned char src[])
{
	try
	{
		m_ComputeShader_obj.load_from_cstr(
	...
	attach_shader(m_ComputeShader_obj);
}

void
ProgramComputeShader::initialize_program_obj()
{
	try
	{
		std::set<std::string> defines;
		m_ComputeShader_obj.compile(defines);
...
ProgramDrawShader::ProgramDrawShader(const unsigned char vs_src[], const unsigned char gs_src[], const unsigned char fs_src[], std::set<std::string> defines)
: glProgram()
{
	initialize_shader_obj(vs_src, gs_src, fs_src);
	initialize_program_obj(defines);
}


void
ProgramDrawShader::initialize_shader_obj(const unsigned char vs_src[], const unsigned char gs_src[], const unsigned char fs_src[])
{
	use_geometryShader = true;
	if (gs_src == NULL)
		use_geometryShader = false;
	try
	{
		m_VertexShader_obj.load_from_cstr(
			reinterpret_cast<char const*>(vs_src));

		if (use_geometryShader)
		m_GeometryShader_obj.load_from_cstr(
			reinterpret_cast<char const*>(gs_src));

		m_FragmentShader_obj.load_from_cstr(
			reinterpret_cast<char const*>(fs_src));
	}
	catch (file_open_error const& e)
	{
		std::cerr << "Error: Failed to open shader file." << std::endl
	...
	attach_shader(m_VertexShader_obj);

	if (use_geometryShader)
	attach_shader(m_GeometryShader_obj);

	attach_shader(m_FragmentShader_obj);
}

void
ProgramDrawShader::initialize_program_obj(std::set<std::string> defines)
{
	try
	{
		m_VertexShader_obj.compile(defines);

		if (use_geometryShader)
		m_GeometryShader_obj.compile(defines);

		m_FragmentShader_obj.compile(defines);
