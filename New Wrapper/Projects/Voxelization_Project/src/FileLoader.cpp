...
void FileLoader::load_triangle_mesh_raw(std::string const& filename, std::vector<Eigen::Vector3f>& vertices, std::vector<std::array<unsigned int, 3> >& faces, std::vector<Eigen::Vector3f>& normals)
{
	...
	Utility::set_vertex_normals_from_triangle_mesh(vertices, faces, normals);
}


void FileLoader::load_raw(std::string const& filename, std::vector<Eigen::Vector3f>& vertices, std::vector<std::array<unsigned int, 3> >& faces, std::vector<Eigen::Vector3f>& normals)
{
...
Eigen::Vector3f FileLoader::calculateMassCenter(std::vector<Eigen::Vector3f>& positions){
...
Eigen::Matrix3f FileLoader::calculateInertiaTensor(std::vector<Eigen::Vector3f>& positions, Eigen::Vector3f center, float pointMass){
...
int FileLoader::vertex_cb(p_ply_argument argument) {
	long value_index;
	void *pdata;
	std::vector<Eigen::Vector3f> * vertexArray;
	ply_get_argument_user_data(argument, &pdata, &value_index);
	vertexArray = static_cast <std::vector<Eigen::Vector3f>  *>(pdata);
...
int FileLoader::face_cb(p_ply_argument argument) {
	...
	void *pdata;
	std::vector<std::array<unsigned int, 3> >* faceArray;
	ply_get_argument_user_data(argument, &pdata, NULL);
	faceArray = static_cast <std::vector<std::array<unsigned int, 3> > *>(pdata);
...
bool FileLoader::load_ply(std::string const& filename, std::vector<Eigen::Vector3f>& vertices, std::vector<std::array<unsigned int, 3> >& faces, std::vector<Eigen::Vector3f>& normals)
{
	std::ostringstream fqfn;
	fqfn << path_resources;
	fqfn << filename;
